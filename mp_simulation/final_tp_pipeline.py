...
def allreduce(x, mpu, op=dist.ReduceOp.SUM, async_op=False):
    handle = dist.all_reduce(x, op=op, group=mpu.get_model_parallel_group(), async_op=async_op)
    return x, handle
...
class ParallelMLP(torch.nn.Module):
    ...
    def forward(self, x, async_op=False):
        if async_op:
            chunks = [x]
        else:
            chunk_size = x.shape[0] // 2 
        ...
        with torch.cuda.stream(self.compute_stream):
            ...
            for chunk in chunks:
                ...
                if self.reduce_scatter:
                    assert False, "code path not implemented"
                    prev_out = out
                    ...
                    out, handle = allreduce(out, self.mpu, async_op=async_op)
                    #if not async_op:
    ...
    def profile_stats(self):
        ...
        print(f"Comm time = {comm_time:.2f} ms / Total Time = {total_time:.2f} ms | Comm % = {comm_time*100/total_time:.2f}")
        return
...
if __name__ == "__main__":
    ...
    sq_len=2048
    flops = 16 * args.mbs * sq_len * hsize ** 2 / tp
    num_batches = 40
    ...
    for iter_no in range(num_batches):
        ...
        with torch.no_grad():
            model(batch, async_op=args.async_op)
        torch.cuda.synchronize()
        batch_end = time.time()
        batch_time = batch_end - batch_start # batch_start.elapsed_time(batch_end) / 1000
        tflops_per_gpu = flops / 1e12 / batch_time
        if dist.get_rank() == 0:
            print(f"Iteration {iter_no+1} : TFLOPs: {tflops_per_gpu} | Batch time = {batch_time*1000} ms")
            model.profile_stats()
