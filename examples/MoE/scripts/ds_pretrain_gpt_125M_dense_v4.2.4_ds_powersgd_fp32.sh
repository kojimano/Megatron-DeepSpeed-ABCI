...
# Loco configs
POWERSGD_ENABLED="true"
POWERSGD_RANK=4
POWERSGD_RATE=10
POWERSGD_ITER=2
POWERSGD_START_STEP=2000
...
host="${HOSTNAME}"
NAME="gpt3-with-v0-v4.2.4.1-ds-powersgd-pr${POWERSGD_RANK}-pra${POWERSGD_RATE}-pi${POWERSGD_ITER}-ps${POWERSGD_START_STEP}-fp32-pile-${MODEL_SIZE}B-lr-${LR}-minlr-${MIN_LR}-bs-${GLOBAL_BATCH_SIZE}-gpus-${NUM_GPUS}-zero-${ZERO_STAGE}-mp-${MP_SIZE}-pp-${PP_SIZE}"
if [ "${NO_PP}" = "true" ]; then
    NAME="${NAME}-no_pp"
fi
...
CHECKPOINT_PATH_LOAD="/blob/users/minjiaz/project/moe/checkpoints/gpt3-with-v0-v4.2.4.1-ds-powersgd-pr4-pra10-pi2-ps2000-fp32-pile-0.125B-lr-6.0e-4-minlr-6.0e-5-bs-256-gpus-16-zero-0-mp-1-pp-1-no_pp/"
...
template_json="ds_config_gpt_powersgd_TEMPLATE.json"
...
sed "s/CONFIG_BATCH_SIZE/${GLOBAL_BATCH_SIZE}/" ${template_json} \
    ...
    | sed "s/CONFIG_POWERSGD_ENABLED/${POWERSGD_ENABLED}/" \
    | sed "s/CONFIG_POWERSGD_RANK/${POWERSGD_RANK}/" \
    | sed "s/CONFIG_POWERSGD_RATE/${POWERSGD_RATE}/" \
    | sed "s/CONFIG_POWERSGD_ITER/${POWERSGD_ITER}/" \
    | sed "s/CONFIG_POWERSGD_START_STEP/${POWERSGD_START_STEP}/" \
      > ${config_json}
else
sed "s/CONFIG_BATCH_SIZE/${GLOBAL_BATCH_SIZE}/" ${template_json} \
    | sed "s/CONFIG_MBSIZE/${BATCH_SIZE}/" \
