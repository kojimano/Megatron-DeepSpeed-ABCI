...
host="${HOSTNAME}"
NAME="gpt3-with-v3-ddp-pile-${MODEL_SIZE}B-lr-${LR}-minlr-${MIN_LR}-bs-${GLOBAL_BATCH_SIZE}-gpus-${NUM_GPUS}-zero-${ZERO_STAGE}-mp-${MP_SIZE}-pp-${PP_SIZE}"
if [ "${NO_PP}" = "true" ]; then
    NAME="${NAME}-no_pp"
fi
...
run_cmd="deepspeed ${DIR}/../../../pretrain_gpt.py ${megatron_options} ${data_options} &> ${OUTPUT_BASEPATH}/log/${NAME}_${host}.log"
echo ${run_cmd}
eval ${run_cmd}
set +x
