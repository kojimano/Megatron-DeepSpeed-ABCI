...
host="${HOSTNAME}"
NAME="gpt3-with-v0-v4.2.6.1-ds-powersgd-pr${POWERSGD_RANK}-pra${POWERSGD_RATE}-pi${POWERSGD_ITER}-ps${POWERSGD_START_STEP}-fp32-pile-${MODEL_SIZE}B-lr-${LR}-minlr-${MIN_LR}-bs-${GLOBAL_BATCH_SIZE}-gpus-${NUM_GPUS}-zero-${ZERO_STAGE}-mp-${MP_SIZE}-pp-${PP_SIZE}"
if [ "${NO_PP}" = "true" ]; then
    NAME="${NAME}-no_pp"
fi
