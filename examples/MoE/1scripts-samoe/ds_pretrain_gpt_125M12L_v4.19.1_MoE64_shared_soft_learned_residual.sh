...
host="${HOSTNAME}"
NAME="gpt-${MODEL_SIZE}B-v4.19.1-shared-soft-learned-residual-lr-${LR}-minlr-${MIN_LR}-bs-${GLOBAL_BATCH_SIZE}-gpus-${NUM_GPUS}-mp-${MP_SIZE}-pp-${PP_SIZE}"
...
deepspeed_options=" \
		    --deepspeed \
            --include worker-1:0,1,2,3,4,5,6,7 \
